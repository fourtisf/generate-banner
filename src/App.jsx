...
function App() {
  ...
  // FUNGSI BARU: Menghapus gambar yang sudah diunggah
  const handleRemoveLogo = (setCoins, coinsState, index, e) => {
    e.preventDefault();
    e.stopPropagation(); // Mencegah jendela file manager terbuka
    const newCoins = [...coinsState];
    newCoins[index] = { ...newCoins[index], logo: null };
    setCoins(newCoins);

    // Reset value dari input file agar file yang sama bisa diunggah lagi
    const inputElement =
                    ...
                    <div className="dropzone-content">
                      {coin.logo ? (
                        <>
                          ...
                          />
                          <span style={{ fontSize: "10px", color: "#00ffaa" }}>
                            Terisi
                          </span>
                        </>
                      ) : (
                        <>
                          ...
                          <span style={{ fontSize: "12px" }}>Drag Logo</span>
                        </>
                      )}
                    </div>
                    {/* TOMBOL DELETE MUNCUL JIKA ADA GAMBAR */}
                    {coin.logo && (
                      <button
